...
public class USBHelper {
    ...
    public UsbDevice getUsbDevice(int vendorId, int productId) {
        ...
        while (deviceIterator.hasNext()) {
            ...
            Timber.d("vendorID--" + device.getVendorId() + "ProductId--" + device.getProductId());
    ...
    public int connection(int vendorId, int productId) {
        ...
        //3)查找设备接口
        if (usbDevice == null) {
            Timber.d("未找到目标设备，请确保供应商ID" + vendorId + "和产品ID" + productId + "是否配置正确");
            return statue;
        }
        ...
        for (int i = 0; i < usbInterface.getEndpointCount(); i++) {
            ...
            switch (ep.getType()) {
                ...
                case UsbConstants.USB_ENDPOINT_XFER_CONTROL://控制
                    epControl = ep;
                    Timber.d("find the ControlEndPoint:" + "index:" + i + "," + epControl.getEndpointNumber());
                    break;
                case UsbConstants.USB_ENDPOINT_XFER_INT://中断
                    if (ep.getDirection() == UsbConstants.USB_DIR_OUT) {//输出
                        epIntEndpointOut = ep;
                        Timber.d("find the InterruptEndpointOut:" + "index:" + i + "," + epIntEndpointOut.getEndpointNumber());
                    }
                    if (ep.getDirection() == UsbConstants.USB_DIR_IN) {
                        epIntEndpointIn = ep;
                        Timber.d("find the InterruptEndpointIn:" + "index:" + i + "," + epIntEndpointIn.getEndpointNumber());
                    }
                    break;
                default:
                    break;
        ...
        if (conn.claimInterface(usbInterface, true)) {
            ...
            Timber.d("设备serial number：" + mySerial);
