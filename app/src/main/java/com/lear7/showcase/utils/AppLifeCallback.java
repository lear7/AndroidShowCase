...
public class AppLifeCallback implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityStarted(@NonNull Activity activity) {
        Timber.d(activity.getLocalClassName() + "onActivityStarted");
        // 添加到Activity栈
        ActivityStackManager.getInstance().addActivity(activity);
        appCount++;
        // 统计前台运行Activity个数
    ...
    public void onActivityPaused(@NonNull Activity activity) {
        Timber.d(activity.getLocalClassName() + "onActivityPaused");
    }

    @Override
    public void onActivityStopped(@NonNull Activity activity) {
        Timber.d(activity.getLocalClassName() + "onActivityStopped");
        ActivityStackManager.getInstance().removeActivity(activity);

        appCount--;
        if (!isForgroundAppValue()) {
            isForground = false;
            Timber.d("app into background ");
        }
    }
